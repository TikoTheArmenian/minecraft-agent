...
function setup(t,fetchImpl){const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mc-chat-'));t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));const sent=[];const bot={username:'Marc',chat:s=>sent.push(s),whisper:(u,s)=>sent.push(`${u}: ${s}`)};const agent={dataDir:dir,bot,epoch:1,state:{task:{status:'running',label:'Planting wheat'},inventory:[]},publish(){},log(){},say(){}};const chat=new LlmChat(agent,{fetchImpl});chat.configure({enabled:true,model:'test-model',apiKey:'test-secret'});return {chat,agent,bot,sent}}
