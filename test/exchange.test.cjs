...
const { Exchange, parseExchange, plan, eligible } = require('../src/exchange.cjs')
const { receiveControl } = require('../src/skill-chat.cjs')
const { fixture, Vec3 } = require('./helpers/survival-fixture.cjs')
function setup(t) {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mc-exchange-'))
  const a = fixture(), b = fixture()
  for (const [f, username, id] of [[a, 'Marc', 1], [b, 'Jerry', 2]]) {
  ...
  Exchange.prototype.move = async function(goal) {
    this.check()
    if (Number.isFinite(goal.x)) this.bot.entity.position = new Vec3(goal.x + 0.5, goal.y, goal.z + 0.5)
  }
  t.after(() => {
    Exchange.prototype.move = moves
    a.agent.stop(false); b.agent.stop(false)
    fs.rmSync(dataDir, { recursive: true, force: true })
  })
  let entityId = 100
  const transfers = []
  for (const [from, to] of [[a, b], [b, a]]) from.bot.toss = async (type, metadata, count) => {
    const name = from.bot.registry.items[type].name
    transfers.push({ from: from.agent.username, name, count })
    from.add(name, -count)
    const drop = { id: ++entityId, name: 'item', position: from.bot.entity.position.clone(),
...
async function settled(...agents) {
  for (let i = 0; i < 1000 && agents.some(a => a.workActive); i++) await new Promise(r => setTimeout(r, 1))
  assert.ok(agents.every(a => !a.workActive), 'both task locks must settle')
}
...
test('automatic decision preserves reserves and chooses reciprocal surplus or a one-way gift', t => {
  const { a, b } = setup(t)
  a.add('bread', 32); b.add('oak_log', 40)
  const trade = plan(a.agent, b.agent, {})
  assert.deepEqual(trade.legs.map(l => [l.name, l.count]), [['bread', 16], ['oak_log', 16]])
  a.add('bread', -16)
...
test('two-way exchange confirms each leg and then releases both task locks', async t => {
  const { a, b, transfers } = setup(t)
  a.add('wheat', 24); b.add('oak_log', 16)
  a.agent.command('trade Jerry 16 wheat for 8 oak_log')
  await settled(a.agent, b.agent)
  assert.deepEqual(transfers.map(l => l.name), ['wheat', 'oak_log'])
...
test('a peer with an active production skill is never stopped to accept a gift', async t => {
  const { a, b, transfers } = setup(t)
  let cancelled = false
  b.agent.activeWork = { cancel() { cancelled = true } }
  a.add('dirt', 16)
...
test('both sides are revalidated after travel before the first handoff', async t => {
  const { a, b, transfers } = setup(t)
  a.add('wheat', 16); b.add('oak_log', 8)
  const meet = Exchange.prototype.meet
  Exchange.prototype.meet = async function() { b.add('oak_log', -8) }
  t.after(() => { Exchange.prototype.meet = meet })
  a.agent.command('trade Jerry 16 wheat for 8 oak_log')
  await settled(a.agent, b.agent)
  assert.equal(transfers.length, 0)
  assert.equal(a.agent.state.task.status, 'failed')
...
test('Stop on the recipient cancels the giver before the first toss', async t => {
  ...
  a.add('dirt', 16)
  const meet = Exchange.prototype.meet
  Exchange.prototype.meet = async function() { b.agent.stop(); this.check() }
  t.after(() => { Exchange.prototype.meet = meet })
  a.agent.command('give Jerry 16 dirt')
  await settled(a.agent, b.agent)
  assert.equal(transfers.length, 0)
...
test('unconfirmed first handoff prevents the return trade and is not retried', async t => {
  ...
  assert.equal(a.agent.state.task.exchange.transfers[0].status, 'unconfirmed')
...
test('automatic Exchange runs a reciprocal plan without explicit items', async t => {
  ...
  await settled(a.agent, b.agent)
  assert.deepEqual(transfers.map(l => [l.name, l.count]), [['bread', 16], ['oak_log', 16]])
...
test('unrelated collection events and inventory gains cannot confirm a handoff', async t => {
  ...
  const old = { id: 77, name: 'item', position: a.bot.entity.position.clone(), getDroppedItem: () => ({ name: 'dirt', count: 16 }) }
  b.bot.entities[old.id] = old
  a.bot.toss = async () => {
    a.add('dirt', -16); b.add('dirt', 16)
    b.bot.emit('playerCollect', b.bot.entity, old)
  }
  const pause = Exchange.prototype.pause
  Exchange.prototype.pause = async function() { this.cancel(); this.check() }
  t.after(() => { Exchange.prototype.pause = pause })
  a.agent.command('give Jerry 16 dirt')
  await settled(a.agent, b.agent)
  assert.equal(a.agent.state.task.counts.given, 0)
  assert.equal(a.agent.state.task.status, 'partial')
...
test('disconnecting the recipient mid-meeting releases both exchange locks', async t => {
  ...
  a.add('dirt', 16)
  const meet = Exchange.prototype.meet
  Exchange.prototype.meet = async function() { b.agent.disconnect(); this.check() }
  t.after(() => { Exchange.prototype.meet = meet })
  a.agent.command('give Jerry 16 dirt')
  await settled(a.agent, b.agent)
  assert.equal(transfers.length, 0)
...
test('Exchange metadata exposes its controls and progress to the dashboard', () => {
  const meta = require('../src/skills.cjs').publicSkills().find(s => s.type === 'exchange')
...
test('fresh airborne drops wait to land before the recipient walks to collect them', async t => {
  const { a, b } = setup(t)
  a.add('dirt', 16)
  let drop, pauses = 0, pickups = 0
  a.bot.toss = async () => {
    a.add('dirt', -16)
    drop = { id: 300, name: 'item', position: b.bot.entity.position.offset(0, 1.3, 0),
      getDroppedItem: () => ({ name: 'dirt', count: 16 }) }
    b.bot.entities[drop.id] = drop
    b.bot.emit('entitySpawn', drop)
  }
  const pause = Exchange.prototype.pause, move = Exchange.prototype.move
  ...
  t.after(() => { Exchange.prototype.pause = pause; Exchange.prototype.move = move })
  a.agent.command('give Jerry 16 dirt')
  await settled(a.agent, b.agent)
