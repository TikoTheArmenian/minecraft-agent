...
function findJar(explicit) {
  return candidateJars(explicit).find((file) => {
    try {
...
function strip(ref) {
  return String(ref).replace(/^minecraft:/, '')
}
  ...
  try {
    ...
    const loadModel = (ref) => {
      ...
      if (!ref.startsWith('builtin/')) {
        try {
          model = JSON.parse(fs.readFileSync(path.join(assets, 'models', `${ref}.json`), 'utf8'))
        } catch {
          model = null
        }
      }
    ...
    const resolveChain = (itemName) => {
      ...
      const lookup = (value) => {
        for (let hops = 0; typeof value === 'string' && value.startsWith('#') && hops < 12; hops++)
    ...
    const chest = (itemName) => {
      ...
      return {
        ...
        h: 0.875,
        d: 0.875,
        top: { file, size: [14, 14], draw: [[28, 0, 14, 14, 0, 0]] },
        front: chestFace(file, 42),
        side: chestFace(file, 28),
      }
    }
    const describe = (itemName) => {
      ...
      if (names.some((name) => /^(item\/generated|item\/handheld|builtin\/generated)/.test(name))) {
        const layers = Object.keys(textures)
          ...
          .sort()
          .map((key) => texture(lookup(textures[key])))
          .filter(Boolean)
        if (!layers.length) return null
        // These grayscale plant sprites are colored by game code, not by a model tintindex.
        return CODE_TINTED.has(itemName)
          ? { type: 'flat', layers, tint: tintFor(itemName) }
          : { type: 'flat', layers }
      }
      const withElements = chain.find(
        (entry) => Array.isArray(entry.model?.elements) && entry.model.elements.length,
      )
      if (withElements) {
        const box = withElements.model.elements
          .map((element) => {
            ...
            return {
              ...
              area: size[0] * size[1] + size[1] * size[2] + size[0] * size[2],
            }
          })
        ...
        if (faces.up && box.size[0] >= 8 && box.size[2] >= 8 && box.size[1] >= 1) {
          ...
          if (top || sideTexture) {
            ...
            for (const [key, axis] of [
              ...
              if (box.size[axis] !== 16) entry[key] = Math.round((box.size[axis] / 16) * 100) / 100
        ...
        const flat =
          sideTexture || texture(lookup(faces.up?.texture)) || texture(lookup(faces.down?.texture))
        if (flat) {
    ...
    log(
      `Item icons: ${Object.keys(items).length} of ${registry.itemsArray.length} items from ${path.basename(jar)}; ${missing.length} without a sprite.`,
    )
    return manifest
  } finally {
...
function ensureTextures({ force = false, jar, log = () => {} } = {}) {
  try {
    ...
    if (!found) {
      log(
        `Item icons unavailable: no Minecraft ${VERSION} client jar found. Set MINECRAFT_JAR or run: node scripts/extract-textures.cjs /path/to/${VERSION}.jar`,
      )
...
if (require.main === module) {
  ...
  const result = ensureTextures({
    force: true,
    jar: args.find((arg) => !arg.startsWith('--')),
    log: console.log,
  })
