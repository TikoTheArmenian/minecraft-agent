...
class WalkingSearchGoal extends goals.Goal {
  constructor(goal, weight = 2) { super(); this.goal = goal; this.weight = weight }
  heuristic(node) { return this.weight * this.goal.heuristic(node) }
...
function installReliableGoto(bot) {
  ...
  bot.pathfinder.goto = (goal) =>
    new Promise((resolve, reject) => {
      let settled = false, segmentEndpoint = null
      const finish = (error) => {
        if (settled) return
        settled = true
      ...
      const advance = () => {
        ...
        if (!reached(bot, new goals.GoalBlock(segmentEndpoint.x, segmentEndpoint.y, segmentEndpoint.z))) return
        finish(Object.assign(failure('PartialRoute', 'Useful route segment reached; continue the destination search.'), {
          endpoint: segmentEndpoint, followed: true,
        }))
...
class TravelMovements extends Movements {
  ...
  getBlock(pos, dx, dy, dz) {
    ...
    if (b.name?.endsWith('_door')) { b.physical = false; b.safe = false }
    if (
      this.surfaceJump &&
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    if (b.name.endsWith('_door')) {
      if (props.half !== 'lower' || head?.name !== b.name || head.getProperties().half !== 'upper') return
    } else if (!head || head.boundingBox !== 'empty') return
    if (!this.getBlock(node, dir.x, -1, dir.z).physical || this.exclusionStep(b) >= 100) return
    const interactions = props.open ? [] : [{x:b.position.x,y:b.position.y,z:b.position.z,dx:0,dy:0,dz:0,useOne:true}]
    neighbors.push(new Move(b.position.x,b.position.y,b.position.z,node.remainingBlocks,1 + interactions.length,[],interactions))
  }
  getNeighbors(node) {
    return super.getNeighbors(node).filter(next => {
      ...
      for (let y = Math.floor(support.height); y < support.height + 1.8; y++) {
        ...
        if (!b || b.shapes.some(s => s[3] > .2 && s[0] < .8 && s[5] > .2 && s[2] < .8 && y+s[4] > support.height && y+s[1] < support.height+1.8)) return false
      }
      return true
    })
  }
...
class Travel {
  ...
  async segments(goal) {
    ...
    const searchGoal = goal.heuristic(this.bot.entity.position) >= 24 ? new WalkingSearchGoal(goal) : goal
    if (searchGoal !== goal) this.log('travel.search', 'Using weighted A* for a long walk; favoring progress over the shortest possible route.')
    for (let n = 0; n < 32; n++) {
      this.check()
      try {
        ...
        this.phase(
          'segment',
          `Continuing after route section ${n + 1}/32`,
        )
    ...
    try {
      ...
      await this.timed(
        () => {
          ...
          return this.bot._placeBlockWithOptions(current.ref, step.face, {
            forceLook: 'ignore',
            swingArm: 'right',
          })
        },
...
module.exports = { Travel, TravelMovements, BUILDING_BLOCKS, reached, installReliableGoto, WalkingSearchGoal }
