...
class WheatFarm extends Survival {
  ...
  async recoverAir() {
    ...
    try {
      ...
      if (this.bot.oxygenLevel < 18)
        throw new Error('Could not restore air after surfacing. Move Marc to open water or land.')
  ...
  async attempt(label, fn) {
    try {
      ...
      if (error.fatal || error.code === 'AIR_RECOVERY' || this.cancelled()) throw error
  ...
  // Refill seeds, prepare level ground, then till and plant every eligible irrigated soil block.
  async expand() {
    ...
    if (this.count('wheat_seeds') < 2) {
      ...
      await this.attempt('Find planting seeds', () =>
        this.gather(
          ['short_grass', 'tall_grass', 'fern'],
          () => this.count('wheat_seeds') >= 8,
          'wheat seeds',
          12,
          false,
        ),
    ...
    if (this.count('wheat_seeds') < 2) {
      this.plan.expansion = 'Need more wheat seeds before expanding.'
      return
    }
    ...
    if (!hoe) {
      ...
      await this.sticks(2)
      await this.planks(2)
      await this.craft('wooden_hoe', table)
      hoe = 'wooden_hoe'
    }
    const spots = this.find(
      ...
      (b) =>
        b.position.y === level &&
        inside(this, b.position) &&
        (isAir(this.bot.blockAt(b.position.offset(0, 1, 0))) ||
          vegetation(this.bot.blockAt(b.position.offset(0, 1, 0)))) &&
    ...
    for (const soil of nearbyFirst(this, spots)) {
      ...
      if (this.counts.planted - before >= 32) break
      ...
      await this.attempt('Expand farm', async () => {
        ...
        if (vegetation(above)) {
          await this.approach(above.position)
          await this.dig(above.position, above.name)
        }
        if (soil.name !== 'farmland') await this.till(soil.position, hoe)
        await this.approach(soil.position)
        await this.plant(soil.position, CROPS.wheat)
      })
    }
  ...
  async extendShore() {
    ...
    if (!candidates.length) {
      this.plan.expansion = 'No supported, irrigated expansion or repair sites at the farm level.'
      return
    }
    this.decide(
      `Repairing and extending the level farm at Y=${layout(this).y}; gathering dirt outside the protected farm.`,
    )
    if (this.count('dirt') < 12)
      await this.attempt('Gather expansion dirt', () =>
        this.gather(
          ['dirt', 'grass_block'],
          () => this.count('dirt') >= 12,
    ...
    if (!this.count('dirt')) {
      this.plan.blocker = 'Need reachable dirt or grass blocks to extend the farm.'
      return
    }
    ...
    for (const target of nearbyFirst(this, water)) {
      ...
      const face = faces.find((d) =>
        ['dirt', 'grass_block', 'farmland', 'stone', 'cobblestone'].includes(
          this.bot.blockAt(target.position.minus(d))?.name,
        ),
      )
      if (!face) continue
      const placed = await this.attempt('Place farm ground', async () => {
        const refPos = target.position.minus(face)
        await this.approach(refPos)
        await this.equip(this.item('dirt'))
        await this.timed(
        ...
        if (
          !(
            isAir(this.bot.blockAt(target.position)) ||
            (this.bot.blockAt(target.position)?.name === 'water' &&
              this.irrigationRemains(target.position, true))
          ) ||
          this.bot.heldItem?.name !== 'dirt'
        )
          throw new Error('Farm-ground placement changed.')
        new Travel(this, 15000).placementValid(
          { pos: target.position, ref: refPos, face },
          'dirt',
          { farmSupport: true },
        )
        const dirt = this.bot.registry.blocksByName.dirt,
          ack = watchBlock(
            this.bot,
            target.position,
      ...
      this.controller.signal,
        ...
        try {
          await this.timed(
            () =>
              this.bot._placeBlockWithOptions(this.bot.blockAt(refPos), face, {
                forceLook: 'ignore',
                swingArm: 'right',
              }),
            7000,
          ...
          built++
          this.plan.groundAdded = (this.plan.groundAdded || 0) + 1
          this.counts.groundAdded = (this.counts.groundAdded || 0) + 1
          this.sync()
        } finally {
          ack.cleanup()
        }
    ...
    if (built)
      this.plan.expansion = `Added or repaired ${built} level ground blocks; planting available plots next.`
    if (!built)
      this.plan.blocker =
        'Expansion needs reachable surface water, solid shore support, and dirt. No new ground could be placed this pass.'
  }
    ...
    for (const support of supports.slice(0, 8)) {
      ...
      if (chest) {
        if (this.agent.colony?.enabled)
          await require('./storage.cjs').manage(this, chest.position, 'food')
  ...
  async deposit(chest) {
    ...
    try {
      ...
      require('./farm-storage.cjs').remember(this, chest, window)
      window.close()
    }
  }
  async store() {
    if (this.agent.colony?.enabled) {
      const shared = require('./storage.cjs')
  ...
  async cycle() {
    ...
    if (this.count('wheat_seeds') < 2) await require('./farm-storage.cjs').restockSeeds(this)
    ...
    await require('./building-supplies.cjs').ensure(this)
