...
const { skills, parseSkill } = require('../skills/registry.cjs')
...
function parseControl(text) {
  ...
  const name = s.replace(
    /^(?:switch (?:my |your )?skill to|switch to|change (?:my |your )?skill to|start|run|enable|turn on) /,
    '',
  )
  const skill = skills.find((skill) =>
    [skill.type.toLowerCase(), skill.label.toLowerCase(), ...skill.aliases].includes(name),
  )
  if (skill) return { type: 'controlSkill', skill: skill.type }
  const command = parseSkill(name)
  return command ? { type: 'controlSkill', skill: command.type, command } : null
}
...
function receiveControl(agent, bot, username, message, whisper = false) {
  if (
    ...
    Object.values(agent.fleet || {}).some((a) => a.username === username)
  )
    return false
  ...
  try {
    ...
    if (!control) {
      try {
        ...
        // Ordinary conversation still falls through; malformed operational requests get a useful rejection.
        if (
          !/^(?:start|switch|run|mine|farm|smelt|flatten|craft|give|trade|go to|goto|move to|find|save|forget|storage|manage|create|reconcile|goal|objective|set goal|supervisor)\b/i.test(
            text,
          )
        )
          return false
    ...
    const result = agent.command(text, { source: 'human', actorId: username })
    reply =
      typeof result === 'string'
        ? result
        : agent.state.messages.length > before
          ? agent.state.messages.at(-1).text
          : 'Command accepted.'
  } catch (error) {
    reply = error.message
  }
  try {
    ...
    /* A failed reply must not interrupt a skill. */
  }
  return true
}
