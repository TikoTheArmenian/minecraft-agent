...
function usageOf(usage) {
  ...
  if (![input, output, cached, cacheWrite, reasoning].every(count) || cached + cacheWrite > input || reasoning > output) return null
  return { input, cached, cacheWrite, output, reasoning }
}
function estimate(model, tier, usage) {
  if (!usage) return { pricingStatus: 'unknown_usage', costNano: null, price: null }
  ...
  const price = { ...base, input: base.input * (long ? 2 : 1), cached: base.cached * (long ? 2 : 1),
    cacheWrite: base.cacheWrite * (long ? 2 : 1), output: base.output * (long ? 1.5 : 1),
