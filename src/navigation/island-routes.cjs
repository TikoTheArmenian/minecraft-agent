/**
 * ISLAND RAMP PLANNER: proposes short stair-and-bridge routes from existing land.
 * This file only describes candidate placements. Travel checks whether they lead to the goal and executes them.
 * A generator yields between scan columns so searching does not freeze movement or the website.
 */
  ...
  for (let dx = -12; dx <= 12; dx++)
    for (let dz = -12; dz <= 12; dz++) {
      ...
      for (let dy = -2; dy <= 6; dy++) {
        ...
        if (
          !solid(bot.blockAt(base)) ||
          !air(bot.blockAt(staging)) ||
          !air(bot.blockAt(staging.offset(0, 1, 0)))
        )
          continue
        for (const dir of DIRECTIONS)
          for (let rise = 1; rise <= 4; rise++) {
            ...
            for (let distance = 1; distance <= 8; distance++) {
              const y = base.y + Math.min(distance, rise),
                p = new Vec3(base.x + dir.x * distance, y, base.z + dir.z * distance)
              if (!air(bot.blockAt(p.offset(0, 1, 0))) || !air(bot.blockAt(p.offset(0, 2, 0))))
                break
              if (solid(bot.blockAt(p))) {
                const landing = p.offset(0, 1, 0)
                if (placements.length && goal.heuristic(landing) < goal.heuristic(staging))
                  plans.push({
                    kind: 'island-ramp',
                    placements,
                    staging,
              ...
              if (p.y > previous.y) {
                ...
                if (
                  !air(bot.blockAt(stand.offset(0, 1, 0))) ||
                  !air(bot.blockAt(stand.offset(0, 2, 0)))
                )
                  break
                if (air(bot.blockAt(support)))
                  placements.push({ pos: support, ref: previous, face: dir, stand })
