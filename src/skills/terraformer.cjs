...
const { BUILDING_BLOCKS, TravelMovements } = require('../navigation/travel.cjs')
...
const open = (b) =>
  !!b && (isAir(b) || liquid(b) || (b.boundingBox === 'empty' && !protectedBlock(null, b)))
...
function protectedBlock(bot, block) {
  ...
  if (PROTECTED_NAMES.has(block.name) || PROTECTED_PATTERNS.some((re) => re.test(block.name)))
    return true
    ...
    for (let dx = -1; dx <= 1; dx++)
      for (let dz = -1; dz <= 1; dz++)
        for (const dy of [0, 1])
          if (
            (dx || dz || dy) &&
            bot.blockAt(block.position.offset(dx, dy, dz))?.name === 'farmland'
          )
            return true
  return false
}
...
function faceVisible(bot, eye, ref, face, reach = 4) {
  const target = ref.offset(0.5, 0.5, 0.5).plus(face.scaled(0.5)),
  ...
  const hit = bot.world.raycast(
    ...
    delta.normalize(),
    distance + 0.05,
    (b, iter) => b.position.equals(ref) || !!iter.intersect(b.shapes, b.position),
  )
...
class FillStanceGoal extends goals.Goal {
  ...
  heuristic(node) {
    return Math.max(0, node.distanceTo(this.dest) - 2)
  }
  isEnd(node) {
    if (node.x === this.dest.x && node.z === this.dest.z) return false
    if (
      node.y < this.dest.y - 1 ||
      this.bot.blockAt(node.offset(0, -1, 0))?.boundingBox !== 'block'
    )
      return false
    return faceVisible(
      this.bot,
...
function parseTerraformer(text) {
  const s = String(text || '')
    ...
    .replace(/[.!?]+$/, '')
  const m = s.match(
    /^flatten\s+(-?\d+)[ ,]+(-?\d+)\s+to\s+(-?\d+)[ ,]+(-?\d+)\s+at\s+(?:y\s*=?\s*)?(-?\d+)$/,
  )
  if (!m) {
    if (/^flatten\b/.test(s))
      throw new Error(
        'Use "flatten X1 Z1 to X2 Z2 at Y", for example flatten -600 1150 to -594 1156 at 70.',
      )
    return null
  }
  const [x1, z1, x2, z2, y] = m.slice(1).map(Number)
  if (
    [x1, z1, x2, z2, y].some((v) => !Number.isSafeInteger(v)) ||
    [x1, z1, x2, z2].some((v) => Math.abs(v) > 30000000)
  )
  ...
  const min = { x: Math.min(x1, x2), z: Math.min(z1, z2) },
    max = { x: Math.max(x1, x2), z: Math.max(z1, z2) }
  if (max.x - min.x + 1 > MAX_SIDE || max.z - min.z + 1 > MAX_SIDE)
    throw new Error(
      `Select at most ${MAX_SIDE}×${MAX_SIDE} columns; split a larger area into several jobs.`,
    )
...
const savedTerraformJobs = (jobs) =>
  ...
  Object.values(jobs).every((job) => {
    ...
    return (
      area &&
      [area.min?.x, area.min?.z, area.max?.x, area.max?.z, area.y].every(Number.isSafeInteger) &&
      Math.max(
        Math.abs(area.min.x),
        Math.abs(area.min.z),
        Math.abs(area.max.x),
      ...
      area.max.x - area.min.x < MAX_SIDE &&
      area.max.z - area.min.z < MAX_SIDE &&
      Array.isArray(job.done) &&
      job.done.every((id) => typeof id === 'string' && /^-?\d+,-?\d+$/.test(id)) &&
      [job.cut, job.filled].every((n) => Number.isSafeInteger(n) && n >= 0) &&
      ['running', 'partial', 'cancelled', 'complete'].includes(job.status)
    )
  })

class Terraformer extends ResourceWork {
  constructor(agent, id) {
    ...
    if (this.jobFile)
      this.jobs =
        loadJson(this.jobFile, { validate: savedTerraformJobs, allowLegacy: true }).data || {}
    this.job = null
  ...
  get username() {
    return this.agent.username || this.bot.username || 'Terra'
  }
  ...
  saveJob() {
    ...
    saveJson(this.jobFile, this.jobs, { validate: savedTerraformJobs })
  }
  check() {
    if (!this.cancelled() && this.bot.game.gameMode !== 'survival')
      throw Object.assign(
        new Error(`${this.username} left Survival mode. Switch it back, then run terraform again.`),
        { fatal: true },
      )
    if (!this.cancelled() && Date.now() >= this.deadline)
      throw Object.assign(
        new Error(
          'Terraforming reached its 60-minute limit. Progress is saved; run terraform again to continue.',
        ),
        { fatal: true },
      )
  ...
  // The shared resource gatherer (used by the shared building-supply policy) must never quarry the rectangle.
  safeTarget(block) {
    if (!super.safeTarget(block)) return false
    const a = this.job?.area
    if (
      a &&
      block.position.x >= a.min.x - PROTECT_BUFFER &&
      block.position.x <= a.max.x + PROTECT_BUFFER &&
      block.position.z >= a.min.z - PROTECT_BUFFER &&
      block.position.z <= a.max.z + PROTECT_BUFFER
    )
      return false
    return true
  }
  ...
  carriedFill() {
    return this.bot.inventory
      .items()
      .filter((i) => BUILDING_BLOCKS.includes(i.name))
      .reduce((n, i) => n + i.count, 0)
  }
  // Keep outstanding fill demand out of surplus deposits until the surface is complete.
  updateReserves(outstanding = 0) {
    for (const name of Object.keys(BASE_RESERVES))
      this.reserves[name] = BASE_RESERVES[name] + outstanding
  }
  ...
  survey() {
    ...
    for (let x = a.min.x; x <= a.max.x; x++)
      for (let z = a.min.z; z <= a.max.z; z++) {
        ...
        for (let dx = -PROTECT_BUFFER; dx <= PROTECT_BUFFER && !guard; dx++)
          for (let dz = -PROTECT_BUFFER; dz <= PROTECT_BUFFER && !guard; dz++)
            guard = protectedColumns.get(column(x + dx, z + dz))
        if (guard) {
        ...
        let missing = false
        for (let y = a.y - FILL_BELOW; y <= a.y + CUT_ABOVE + 1 && !missing; y++)
          if (!at(y)) missing = true
        if (missing) {
          unloaded.push(id)
          continue
        }
        if (solid(at(a.y + CUT_ABOVE + 1))) {
          tooTall.push(id)
          continue
        }
        const columnCuts = [],
        ...
        for (let y = a.y + CUT_ABOVE; y > a.y; y--) {
          ...
          if (isAir(b)) continue
          if (liquid(b)) {
            blockedBy = `${b.name} above the target level cannot be removed`
            break
          }
          if (!b.diggable || b.hardness < 0) {
            blockedBy = `${b.name} cannot be mined`
            break
          }
          columnCuts.push({ pos: b.position.clone(), name: b.name })
        }
        if (blockedBy) {
          skipped.push({ x, z, reason: blockedBy })
          continue
        }
        ...
        if (!columnCuts.length && !columnFills.length) {
          done.push(id)
          continue
        }
    ...
    return {
      ...
      columns: (a.max.x - a.min.x + 1) * (a.max.z - a.min.z + 1),
    }
  }
  publishSurvey(s) {
  ...
  async storageHub() {
    ...
    if (!this.agent.colony?.enabled) {
      this.plan.storage = 'Colony storage is disabled; surplus stays in inventory.'
      return null
    }
    try {
  ...
  async budget(s) {
    ...
    if (hub && !this.toolsChecked) {
      ...
      for (const tool of ['iron_pickaxe', 'iron_shovel'])
        if (!require('../messaging/colony-chat.cjs').hasTool(this.bot.inventory.items(), tool)) {
          this.decide(`Fetching a ${tool.replaceAll('_', ' ')} from shared storage.`)
          try {
            await storage.retrieve(this, [tool], 1)
          } catch (error) {
            this.check()
            if (error.fatal) throw error
            this.addIssue(`Tool pickup: ${error.message}`)
          }
        }
    }
    ...
    if (shortfall <= 0 || !hub || this.storageExhausted) {
      this.decide(
        `Budget: ${s.needFill} fill blocks needed, ${carried} carried, about ${s.expectedYield} expected from cuts; ${s.cuts.length} blocks to cut.`,
      )
      return
    }
    const target = Math.min(s.needFill, carried + 128)
    this.decide(
      `Budget: ${s.needFill} fill blocks needed, ${carried} carried; retrieving building blocks from shared storage (target ${target}).`,
    )
    try {
      ...
      throw Object.assign(
        new Error(
          this.hub
            ...
            : `Inventory is full. Empty ${this.username}'s inventory or enable shared storage near this job, then run terraform again.`,
        ),
        { code: 'BLOCKED' },
      )
  }
  ...
  cuttable(b) {
    ...
    const sides = [
      ...
      [0, 0, -1],
      [0, 1, 0],
    ].map((d) => this.bot.blockAt(b.position.offset(...d)))
  ...
  async cutOne(block) {
    ...
    try {
      ...
      this.decide(
        `Cutting ${block.name.replaceAll('_', ' ')} at ${key(pos)} · ${this.job.cut} cut, ${this.job.filled} filled.`,
      )
      await this.approachCut(pos)
      const live = this.bot.blockAt(pos)
      if (!live || isAir(live)) return true
      if (!this.cuttable(live))
        throw new Error(`${live.name} is beside liquid or a protected block; left in place.`)
      ...
      this.checkpoint({ phase: 'cut-confirmed', position: { ...pos } })
      return true
    } catch (error) {
      if (
        error.fatal ||
        ...
        throw error
      this.check()
      const message = `${key(pos)}: ${error.message}`
  ...
  async cutLayers(s) {
    ...
    for (const y of layers) {
      this.check()
      const rows = new Map()
      for (const c of s.cuts.filter((c) => c.pos.y === y))
        (rows.get(c.pos.z) || rows.set(c.pos.z, []).get(c.pos.z)).push(c)
      const order = [...rows.keys()].sort((a, b) => a - b)
      order.forEach((z, index) =>
        rows.get(z).sort((a, b) => (index % 2 ? b.pos.x - a.pos.x : a.pos.x - b.pos.x)),
      )
      for (const z of order) {
        const targets = rows
          .get(z)
          .map((c) => this.bot.blockAt(c.pos))
          .filter((b) => b && !isAir(b))
  ...
  fillItem(surface) {
    ...
    items.sort(
      (a, b) =>
        (surface
          ? Number(b.name === 'dirt') - Number(a.name === 'dirt')
          : Number(a.name === 'dirt') - Number(b.name === 'dirt')) ||
  ...
  supportsFor(dest) {
    ...
    for (const d of [
      ...
      if (solid(n) && !protectedBlock(this.bot, n))
        sides.push({ ref: n.position, face: new Vec3(-d[0], -d[1], -d[2]) })
    }
  ...
  async standNear(ref, dest, face = new Vec3(0, 1, 0)) {
    ...
    if (!goal.isEnd(workingCell(this.bot)))
      throw new Error('No stance with a clear view of the fill face.')
  }
  placementValid(dest, ref, face, name) {
    this.check()
    const target = this.bot.blockAt(dest),
      support = this.bot.blockAt(ref)
    ...
    if (eye.distanceTo(ref.offset(0.5, 0.5, 0.5).plus(face.scaled(0.5))) > 4.5)
      throw new Error('Fill spot is outside placement reach.')
    if (this.bot.heldItem?.name !== name || this.bot.heldItem.count < 1)
      throw new Error('Fill material is no longer equipped.')
  ...
  async placeFill(dest, item) {
    ...
    await this.timed(
      () => this.bot.lookAt(ref.offset(0.5, 0.5, 0.5).plus(face.scaled(0.5))),
      5000,
      `Face the fill spot ${key(dest)}`,
    )
    ...
    const ack = watchBlock(
      this.bot,
      dest,
      (s) => s >= type.minStateId && s <= type.maxStateId,
    ...
    try {
      await this.timed(
        () => {
          ...
          return placeBlockWithOptions(this.bot, current, face, {
            forceLook: 'ignore',
            swingArm: 'right',
          })
        },
        7000,
        `Place ${item.name} at ${key(dest)}`,
      )
      await this.timed(() => ack.promise, 4000, `Confirm fill at ${key(dest)}`)
      if (this.bot.blockAt(dest)?.name !== item.name)
        throw new Error('The server did not confirm the fill block.')
      this.job.filled++
  ...
  async jumpFill(dest, item) {
    ...
    this.decide(
      `Climbing out of ${key(dest)} by placing ${item.name.replaceAll('_', ' ')} under my feet.`,
    )
    await this.equip(item)
    await this.timed(
      () => this.bot.lookAt(dest.offset(0.5, 0, 0.5)),
      5000,
      'Face the floor of the pit',
    )
    this.check()
    ...
    try {
      await this.motionUntil(
        () => this.bot.entity.position.y >= dest.y + 1.01,
        'Jump before placing the block under the feet',
      )
      await this.timed(
        () =>
          placeBlockWithOptions(this.bot, this.bot.blockAt(ref), new Vec3(0, 1, 0), {
            forceLook: 'ignore',
            swingArm: 'right',
          }),
        7000,
        `Place ${item.name} under the feet at ${key(dest)}`,
      )
      await this.timed(() => ack.promise, 4000, `Confirm fill at ${key(dest)}`)
    } finally {
    ...
    if (this.bot.blockAt(dest)?.name !== item.name)
      throw new Error('The server did not confirm the fill block under the feet.')
    this.job.filled++
    this.plan.filled = this.job.filled
    this.counts.placed = (this.counts.placed || 0) + 1
    this.sync()
    ...
    await this.motionUntil(
      () =>
        ...
        this.bot.entity.position.floored().equals(dest.offset(0, 1, 0)),
      'Land on the placed block',
    )
  }
  async fillColumn(x, z, cells) {
    ...
    for (const y of cells) {
      ...
      if (!open(b)) continue
      if (!isAir(b) && !liquid(b))
        throw new Error(`${key(dest)}: ${b.name} must be cut before filling.`)
      ...
      this.decide(
        `Filling ${key(dest)} with ${item.name.replaceAll('_', ' ')} · ${this.job.filled} placed so far.`,
      )
  ...
  async fillColumns(s) {
    ...
    for (const r of [...rings.keys()].sort((x, y) => x - y)) {
      ...
      const anchors = rings
        .get(r)
        .map((c) => this.bot.blockAt(new Vec3(c.x, a.y, c.z)))
        .filter(Boolean)
      for (const anchor of nearbyFirst(this, anchors)) {
        ...
        try {
          ...
          this.checkpoint({ phase: 'fill-column-complete', x: c.x, z: c.z })
        } catch (error) {
          if (
            error.fatal ||
            error.code === 'HANDOFF' ||
            ...
            throw error
          if (error.code === 'NO_MATERIAL') {
            this.addIssue(
              'Ran out of fill material; cutting more or checking storage before continuing.',
            )
            return
          }
          this.check()
          const message = `${column(c.x, c.z)}: ${error.message}`
          this.failures.push(message)
          this.addIssue(message)
        }
      }
  ...
  chooseArea(command) {
    ...
    if (command?.min && command?.max) {
      const area = {
        min: { x: command.min.x, z: command.min.z },
        max: { x: command.max.x, z: command.max.z },
        y: command.y,
      }
    ...
    return {
      area: { min: { x: p.x - 4, z: p.z - 4 }, max: { x: p.x + 4, z: p.z + 4 }, y: p.y - 1 },
      resume: false,
    }
  }
  finish(s, deficit) {
    ...
    const skippedNote =
      (s.skipped.length
        ? ` ${s.skipped.length} column(s) skipped: ${s.skipped
            .slice(0, 3)
            .map((k) => `${k.x},${k.z} (${k.reason})`)
            .join('; ')}${s.skipped.length > 3 ? '…' : ''}.`
  ...
  async execute() {
    for (let pass = 0; pass < 64; pass++) {
      ...
      this.failures = []
      try {
        this.checkpoint({ phase: 'between-terrain-passes' })
        this.check()
        await this.eat()
        ...
        if (s.tooTall.length)
          throw Object.assign(
            new Error(
              `Column ${s.tooTall[0].replace(',', ' ')} has solid blocks more than ${CUT_ABOVE} above Y=${this.job.area.y}: too tall; choose a higher target or smaller area.`,
            ),
            { code: 'REFUSED' },
          )
        if (s.unloaded.length)
          throw new Error(
            `${s.unloaded.length} column(s) are not loaded; move closer to ${s.unloaded[0].replace(',', ' ')}.`,
          )
        ...
        if (
          ...
          s.needFill &&
          !this.carriedFill() &&
          (!this.hub || this.storageExhausted)
        ) {
          this.updateReserves(0)
        ...
        if (this.failures.length && this.job.cut + this.job.filled === before)
          throw new Error(this.failures.at(-1))
        this.stalledPasses = 0
      } catch (error) {
        if (
          error.fatal ||
          error.code === 'HANDOFF' ||
          this.cancelled() ||
          ['CANCELLED', 'REFUSED'].includes(error.code)
        )
          throw error
        ...
        if (this.stalledPasses >= 3)
          throw new Error(
            `No progress after three attempts: ${error.message} The unfinished job is saved; clear the blocker, then run terraform again.`,
            { cause: error },
          )
    ...
    throw new Error(
      'Pass limit reached; the unfinished job is saved. Run terraform again to continue.',
    )
  }
  async run(command = { type: 'terraformer' }) {
    ...
    try {
      ...
      const here = this.bot.entity.position
      for (const corner of [
        [area.min.x, area.min.z],
        [area.max.x, area.max.z],
      ])
        if (
          Math.hypot(corner[0] + 0.5 - here.x, area.y + 1 - here.y, corner[1] + 0.5 - here.z) >
          MAX_REACH
        )
          throw Object.assign(
            new Error(
              `Both corners must be within ${MAX_REACH} blocks of ${this.username}. Move closer to ${area.min.x} ${area.y} ${area.min.z} – ${area.max.x} ${area.y} ${area.max.z}, then start again.`,
            ),
            { code: 'REFUSED' },
          )
      const saved = this.jobs[this.jobKey]
      this.job = resume
        ? saved
        : { area, done: [], cut: 0, filled: 0, status: 'running', createdAt: Date.now() }
      ...
      const cancelled = ['CANCELLED', 'HANDOFF'].includes(reason.code)
      ...
      if (refused && this.job) {
        ...
        this.job.status = cancelled ? 'cancelled' : 'partial'
        this.saveJob()
      }
