...
function parseMobKiller(text) {
  const s = String(text || '')
    ...
    .replace(/[.!?]+$/, '')
  const m = s.match(
    /^(?:start )?(?:hunt mobs|kill mobs|guard here|mob killer)(?: within (\d+)(?: blocks)?)?$/,
  )
  if (!m) return null
  const radius = m[1] ? Number(m[1]) : DEFAULT_RADIUS
  if (!Number.isSafeInteger(radius) || radius < MIN_RADIUS || radius > MAX_RADIUS)
    throw new Error(
      `Hunt mobs within ${MIN_RADIUS}–${MAX_RADIUS} blocks, for example "hunt mobs within 16".`,
    )
...
class MobKiller extends ResourceWork {
  ...
  safety() {
    ...
    if (this.bot.entity.isInLava)
      return `${me} is in lava. The mob killer needs help reaching safety.`
  ...
  publishPlan() {
    ...
    this.plan.weapon =
      this.bot.heldItem && WEAPON.test(this.bot.heldItem.name) ? this.bot.heldItem.name : null
    this.agent.publish()
  }
  // --- target selection -------------------------------------------------------------------
  fromPost(p) {
  ...
  hostilesNearPost() {
    return Object.values(this.bot.entities || {}).some(
      (e) => this.hostile(e) && this.fromPost(e.position) <= this.radius,
    )
  }
  ...
  async arm(target) {
    ...
    if (
      ...
      Date.now() - this.swordAskedAt >= SWORD_RETRIEVE_MS
    ) {
  ...
  inReach(entity) {
    return (
      finite(entity?.position) && this.reachDistance(entity) <= ATTACK_RANGE && this.visible(entity)
    )
  }
  beyondBound(entity) {
  ...
  async engage(target) {
    ...
    if (!weapon && !this.fistsAllowed(name)) {
      this.coolDown(
        target,
        `No melee weapon carried; skipping ${name.replaceAll('_', ' ')} until ${me} has a sword${FIST_TARGETS.has(name) ? ' or full health' : ''}.`,
      )
      this.target = null
    ...
    if (!weapon)
      this.decide(
        `Fighting ${name.replaceAll('_', ' ')} bare-handed at health ${this.bot.health}/20; a sword would be safer.`,
      )
    else
      this.decide(`Engaging ${name.replaceAll('_', ' ')} with ${weapon.name.replaceAll('_', ' ')}.`)
    let hits = 0,
      routeFailures = 0,
      holdUntil = null,
    ...
    try {
      while (true) {
        ...
        if (!finite(live.position)) {
          this.coolDown(
            live,
            `${name.replaceAll('_', ' ')} has no usable position; skipping it for 30 seconds.`,
            true,
          )
          return false
        }
        ...
        if (this.beyondBound(live)) {
          this.coolDown(
            live,
            `${name.replaceAll('_', ' ')} moved beyond ${this.radius + PURSUIT_MARGIN} blocks from the post; returning.`,
          )
          return false
        }
        if (hits >= HIT_BUDGET) {
        ...
        if (Date.now() - started > ENGAGE_MS) {
          this.coolDown(
            live,
            `Spent ${Math.round(ENGAGE_MS / 1000)} seconds on ${name.replaceAll('_', ' ')} without a kill; cooling it down.`,
          )
          return false
        }
        ...
        if (holdUntil) {
          ...
          this.coolDown(
            live,
            `No route to ${name.replaceAll('_', ' ')} and it did not come closer; skipping it for 30 seconds.`,
          )
          return false
        }
        const followed = await this.chase(
          live,
          `Chase ${name} (${round(this.fromBot(live.position))} blocks away)`,
        )
        // Arrived (or interrupted) but still not in reach: let physics and the mob move before re-planning.
        if (followed && this.bot.entities[id] && !this.inReach(this.bot.entities[id]))
          await this.pause(250)
        if (!followed && ++routeFailures >= 2) {
          if (this.fromBot(live.position) <= HOLD_RANGE) {
            holdUntil = Date.now() + HOLD_MS
            this.decide(
              `No route to ${name.replaceAll('_', ' ')} ${round(this.fromBot(live.position))} blocks away; holding ground for ${HOLD_MS / 1000} seconds.`,
            )
            continue
          }
          this.coolDown(
            live,
            `No route to ${name.replaceAll('_', ' ')}; skipping it for 30 seconds.`,
          )
          return false
        }
      }
  ...
  async recordKill(name, position) {
    ...
    this.sync()
    this.decide(
      `Killed ${name.replaceAll('_', ' ')} · ${this.plan.killsTotal} total. Collecting drops.`,
    )
    this.agent.log?.(
      'mob.kill',
      `${this.agent.username} killed ${name} at ${position.floored()}.`,
      'info',
      { taskId: this.id },
    )
  ...
  async retreat(reason) {
    ...
    while (this.bot.health < RESUME_HEALTH && Date.now() < until) {
      ...
      this.decide(
        `Recovering at the post: health ${this.bot.health}/20, resuming at ${RESUME_HEALTH}.`,
      )
  ...
  async avoidCreeper(creeper) {
    ...
    const postLeadsAway =
      this.fromBot(this.origin) > 1 &&
      this.origin.distanceTo(creeper.position) > this.fromBot(creeper.position) + 2
    const point = postLeadsAway ? this.origin : me.plus(direction.scaled(CREEPER_DISTANCE + 3))
    this.decide(
      `Creeper ${round(this.fromBot(creeper.position))} blocks away: backing off without attacking.`,
    )
    try {
  ...
  async storeBatch() {
    ...
    try {
      ...
      if (hub && this.origin.distanceTo(new Vec3(hub.x, hub.y, hub.z)) > HUB_RANGE) {
        this.decide(
          `Storage hub is ${Math.round(this.origin.distanceTo(new Vec3(hub.x, hub.y, hub.z)))} blocks from the post (limit ${HUB_RANGE}); keeping drops for now.`,
        )
      ...
      this.decide(
        moved
          ? `Stored ${moved} items in the shared hub · ${this.plan.stored} total.`
          : 'Storage accepted nothing new; continuing the patrol.',
      )
  ...
  async idle() {
    ...
    this.decide(
      `No hostile mobs within ${this.radius} blocks of the post · ${this.plan.killsTotal} kills. Watching.`,
    )
  ...
  async run(command = {}) {
    ...
    if (this.bot.pathfinder.setMovements)
    ...
    try {
      ...
      while (true) {
        try {
          ...
          if (!this.hostilesNearPost()) {
            this.checkpoint({ phase: 'guard-post-clear' })
