...
const fatal = (w, e) =>
  e.fatal || e.code === 'HANDOFF' || e.code === 'AIR_RECOVERY' || w.cancelled()
...
class TorchSkill extends ResourceWork {
  ...
  async run() {
    ...
    try {
      this.checkpoint({ phase: 'before-lighting-supplies' })
      await getTorches(this, 16)
      this.checkpoint({ phase: 'lighting-supplies-ready' })
      this.task.status = 'succeeded'
      this.agent.say(`Torches ready: ${this.count('torch')} in inventory.`)
    } catch (e) {
      if (e.code === 'HANDOFF') this.task.reasonCode = 'HANDOFF'
      this.task.status = this.cancelled() || e.code === 'HANDOFF' ? 'cancelled' : 'partial'
      this.agent.say(`Get torches stopped: ${this.controller.signal.reason?.message || e.message}`)
      if (e.fatal) this.failure = e
    } finally {
      clearInterval(guard)
