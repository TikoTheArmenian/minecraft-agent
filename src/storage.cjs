...
const { describe, plain, category, reserve } = require('./storage-policy.cjs')
const posKey = (p) => `${p.x},${p.y},${p.z}`
const vector = (p) => new Vec3(p.x, p.y, p.z)
function identity(bot, block) {
  if (block?.name !== 'chest')
    throw new Error('Only ordinary chests are managed in this version.')
...
const total = (items, fp) =>
  items
    .filter((i) => describe(i).fingerprint === fp)
...
async function transfer(w, ctx, kind, fingerprint, wanted, job = null) {
  ...
  for (let step = 0; step < 64 && moved < wanted; step++) {
    ...
    const from =
      kind === 'deposit'
        ? [window.inventoryStart, window.inventoryEnd]
        : [0, window.inventoryStart]
    const to =
      kind === 'deposit'
        ? [0, window.inventoryStart]
        : [window.inventoryStart, window.inventoryEnd]
    ...
    if (!source) break
    const limit =
      source.stackSize || w.bot.registry.items[source.type]?.stackSize || 64
    let dest = -1
    for (let i = to[0]; i < to[1]; i++) {
      const item = window.slots[i]
      if (
        item &&
        describe(item).fingerprint === fingerprint &&
        item.count < limit
      ) {
        dest = i
        break
    ...
    const count = Math.min(
      wanted - moved,
      source.count,
      limit - (window.slots[dest]?.count || 0),
    )
    ...
    if (
      total(window.containerItems(), fingerprint) - beforeChest !==
        direction * count ||
      total(
        window.slots
          .slice(window.inventoryStart, window.inventoryEnd)
      ...
      throw new Error(
        'Transfer is uncertain; chest writes are quarantined for reconciliation.',
      )
...
function eligible(w, containers) {
  return containers
    .filter(
      (c) =>
        c.managed &&
        vector(c.position).distanceTo(w.origin || w.bot.entity.position) <= 80,
    )
    .sort(
      (a, b) =>
  ...
  for (const item of carried(w)) {
    ...
    for (const chest of targets.slice(0, 8)) {
      ...
      const n = await withChest(w, chest.position, (ctx) =>
        transfer(w, ctx, 'deposit', fp, amount),
      )
      moved += n
      amount -= n
    }
  ...
  for (const chest of eligible(w, containers).slice(0, 8)) {
    for (const item of chest.slots.slice(0, 16)) {
      ...
      if (
        cat === chest.category ||
        !containers.some(
          (c) => c.managed && c.category === cat && c.id !== chest.id,
        )
      )
        continue
      const n = await withChest(w, chest.position, (ctx) =>
