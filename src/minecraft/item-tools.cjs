...
function comparisonBlock(bot, block) {
  ...
  if (
    block.material?.startsWith('incorrect_for_') &&
    harvesters.length &&
    harvesters.every((id) => bot.registry.items[id]?.name.endsWith('_pickaxe'))
  )
    return Object.assign(Object.create(block), { material: 'mineable/pickaxe' })
  return block
}
function installToolCompatibility(bot) {
  ...
  bot._client?.on('registry_data', (packet) => {
    if (packet.id === 'minecraft:enchantment')
      serverEnchantments.set(
        bot,
        packet.entries.map((e) => e.key.replace(/^minecraft:/, '')),
      )
  })
  ...
  const digTime = (block) => {
    ...
    const enchants = [
      ...enchantments(bot.heldItem, bot),
      ...enchantments(bot.inventory.slots?.[headSlot], bot),
    ]
    return comparisonBlock(bot, block).digTime(
      bot.heldItem?.type ?? null,
