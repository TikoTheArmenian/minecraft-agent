/**
 * START HERE: starts the local website and exposes its /api endpoints.
 * Each Agent owns one Minecraft player; the fleet list lives in src/fleet.cjs.
...
function createApp(agent, fleet = null) {
  ...
  if (fleet) {
    ...
    app.get('/api/fleet', (req, res) =>
      res.json(
        Object.fromEntries(
          Object.entries(fleet).map(([id, bot]) => [id, { ...bot.state, profile: bot.profile || { id, username: bot.username } }]),
        ),
      ),
    )
  ...
  app.get('/api/storage', async (req, res, next) => {
    try {
      if (!agent.colony.enabled) return res.json({configured:false,containers:[],jobs:[],reservations:[],uncertain:[]})
  ...
  app.post('/api/craft-jobs', async (req,res,next) => {
    try {
      ...
      if(typeof item!=='string' || !require('./crafting.cjs').allowed(item) || !Number.isInteger(quantity) || quantity<1 || quantity>128)
  ...
  app.get('/api/navigation-snapshot', (req, res, next) => {
    try {
      ...
      const { Vec3 } = require('vec3'),
        p = agent.bot.entity.position.floored(),
        blocks = []
      for (let x = -12; x <= 12; x++)
        for (let z = -12; z <= 12; z++)
          for (let y = -4; y <= 10; y++) {
...
if (require.main === module) {
  ...
  server.on('listening', () =>
    console.log(`Control room for ${Object.values(fleet).map((b) => b.username).join(', ')}: http://127.0.0.1:4317`),
  )
  const ticker = setInterval(() => {
    for (const bot of Object.values(fleet)) if (bot.state.connection === 'ready') {
      bot.refresh()
      void bot.coordination.tick().catch(error => bot.log('colony.chat.error',error.message,'warn'))
    }
  }, 500)
