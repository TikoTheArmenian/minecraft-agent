...
const SOIL = ['sand', 'red_sand', 'dirt', 'grass_block', 'podzol', 'coarse_dirt', 'mud', 'rooted_dirt']
...
// Marc's wheat farm and any other crop field are off limits, including their tilled edge.
...
class SugarcaneFarm extends Survival {
  constructor(agent, id) {
    // Survival's constructor publishes its own starter-plan state; keep the previous value.
  ...
  // Survival's check enforces its 20-minute limit and Marc-specific wording; this skill is continuous.
  check() {
    ...
    if (this.bot.game.gameMode !== 'survival')
      throw Object.assign(new Error(`${this.agent.username} left Survival mode. Switch it back, then restart.`), {
        fatal: true,
      })
  }
  safety() {
    ...
    return danger.replaceAll('Marc', this.agent.username)
  }
  addIssue(text) {
    super.addIssue(String(text).replaceAll('Marc', this.agent.username))
  }
  requestAir() {
    if (
      this.recoveringAir ||
  ...
  progressCount() {
    return this.counts.harvested + this.counts.planted + (this.counts.stored || 0) + (this.counts.retrieved || 0)
  }
  ...
  async harvestReady(columns) {
    const bases = columns.filter((c) => c.ready && !this.coolingDown(c.base.position)).map((c) => c.base)
    let attempted = 0
    for (const base of nearbyFirst(this, bases)) {
      ...
      if (this.bot.inventory.emptySlotCount() < 2) {
        ...
        if (this.bot.inventory.emptySlotCount() < 2) {
          this.plan.blocker = 'Inventory is full; sugar cane is left growing until space is available.'
          break
        }
      }
  ...
  async plantMore() {
    ...
    const spots = this.find(SOIL, 48, (b) => plantable(this.bot, b) && !this.coolingDown(b.position))
    ...
    for (const group of [adjacent, others]) {
      for (const soil of nearbyFirst(this, group)) {
        ...
        if (this.counts.planted - before >= PLANT_LIMIT || this.count('sugar_cane') <= RESERVE) return
  ...
  async restock() {
    ...
    await this.attempt('Restock sugar cane from shared storage', async () => {
      ...
      if (!moved) this.plan.expansion = 'Shared storage has no sugar cane to plant; waiting for growth.'
    })
  }
  ...
  async storeBatch(force = false) {
    ...
    await this.attempt('Store sugar cane', async () => {
      this.decide(`Storing surplus sugar cane in shared storage · ${this.plan.stored} stored so far`)
    ...
    try {
      while (true) {
        ...
        if (this.stalledPasses >= 3)
          throw new Error(`No progress after three passes with errors. ${this.plan.blocker || ''}`.trim())
