...
const { BUILDING_BLOCKS } = require('../navigation/travel.cjs')
const CATEGORIES = ['tools', 'wood', 'building', 'food', 'materials', 'overflow']
...
function plain(item) {
  ...
  const pristineDamageOnly =
    nbt?.type === 'compound' &&
    Object.keys(nbt.value || {}).every((k) => k === 'Damage' && nbt.value[k]?.value === 0)
  return (
...
function category(item, registry) {
  ...
  if (
    ...
    /_(sword|pickaxe|axe|hoe|shovel|helmet|chestplate|leggings|boots)$/.test(name)
  )
    return 'tools'
...
function baselineReserve(item, work) {
  ...
  if (['bread', 'cooked_beef', 'baked_potato', 'apple', 'melon_slice'].includes(name)) return 16
  if (BUILDING_BLOCKS.includes(name)) return 128
  return 0
}
function reserve(item, work) {
  const amount = (value) => (typeof value === 'number' && value >= 0 ? Math.ceil(value) : 0)
  // General surplus storage and crafting keep the strongest applicable reserve.
  // Deliberately depositing a crafted/shared output uses storage.store's explicit count.
  const supplies = work.agent?.currentSkillNeeds?.()?.supplies
  return Math.max(
    baselineReserve(item, work),
    amount(supplies?.[item.name]),
    amount(work.reserves?.[item.name]),
  )
}
