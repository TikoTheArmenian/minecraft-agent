...
class Colony {
  ...
  scope(agent) {
    if (!this.url || !this.key)
      throw new Error('Configure SUPABASE_URL and SUPABASE_SECRET_KEY before using storage.')
    ...
    if (!agent.state.dimension) throw new Error('Connect to a dimension before using storage.')
    return {
      label: label.trim(),
  ...
  async resolveWorld(label, agent) {
    ...
    if (!this.resolvedWorlds.has(label)) {
      ...
      const pending = this.request(
        'colony_resolve_world',
        {
          p_label: label,
          p_existing_id: /^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(existing || '')
            ? existing
            : null,
        },
        agent,
  ...
  async call(agent, action, data = {}) {
    ...
    return this.request(
      action.startsWith('hub_')
        ? 'colony_hub_rpc'
        : action.startsWith('reconcile_')
          ? 'colony_reconcile_rpc'
          : 'colony_rpc',
      {
        action,
  ...
  async request(rpc, body, agent = null) {
    ...
    const id = costs?.begin({
      agent: agent?.username || body?.payload?.bot || 'project',
      provider: 'supabase',
      operation: body?.action || rpc,
    })
    let response,
