...
const { describe, plain, reserve } = require('./policy.cjs')
const storage = require('./service.cjs')
const { watchBlock } = require('../minecraft/block-updates.cjs')
const allowed = (name) =>
  /^(?:(?:oak|birch|spruce|jungle|acacia|dark_oak|cherry|mangrove)_(?:planks|sign)|stick|chest|crafting_table|furnace|bread|torch|iron_(?:block|helmet|chestplate|leggings|boots)|(?:wooden|stone|iron)_(?:pickaxe|axe|shovel|hoe|sword))$/.test(
    name,
  )
function planRecipes(bot, item, quantity, carry, shared, needTable = false) {
  if (!allowed(item) || !Number.isInteger(quantity) || quantity < 1 || quantity > 128)
    throw new Error('Craft 1–128 supported plain wood products, basic tools, bread or torches.')
  let state = {
    carry: { ...carry },
    shared: { ...shared },
    withdrawals: {},
  ...
  function produce(name, count, depth = 0, path = []) {
    ...
    const recipes = id === undefined ? [] : bot.recipesAll(id, null, true).slice(0, 64)
    for (const recipe of recipes) {
      ...
      try {
        ...
        for (const delta of recipe.delta.filter((d) => d.count < 0))
          consume(bot.registry.items[delta.id].name, -delta.count * times, depth + 1, [
            ...path,
            name,
          ])
        for (const delta of recipe.delta.filter((d) => d.count > 0)) {
    ...
    throw new Error(`Missing materials for ${name}; inspect shared stock and working reserves.`)
  }
...
function stocks(w, data) {
  ...
  for (const chest of storage.eligible(w, data.containers)) {
    if (!chest.checked_at || Date.now() - Date.parse(chest.checked_at) > 300000) continue
    ...
    for (const item of aggregated.values()) {
      const held = data.reservations
        .filter((r) => r.container === chest.id && r.fingerprint === item.fingerprint)
...
async function place(w, name, hub = null) {
  ...
  for (const p of candidates) {
    ...
    if (
      !clear() ||
      [new Vec3(1, 0, 0), new Vec3(-1, 0, 0), new Vec3(0, 0, 1), new Vec3(0, 0, -1)].some(
        (v) => bot.blockAt(target.plus(v))?.name === 'chest',
      )
    )
      continue
    await w.approach(p)
    if (!clear() || bot.entity.position.distanceTo(target.offset(0.5, 0, 0.5)) < 1.5) continue
...
async function execute(w, job, { storeOutput = true } = {}) {
  const db = (action, data = {}) => storage.call(w, action, { job: job.id, ...data })
  try {
    ...
    let plan = planRecipes(w.bot, job.item, job.quantity, stock.carry, stock.shared)
    if (
      !table &&
      !w.bot.inventory.items().some((i) => i.name === 'crafting_table' && plain(i)) &&
      plan.steps.some((s) => s.recipe.requiresTable)
    )
      plan = planRecipes(w.bot, job.item, job.quantity, stock.carry, stock.shared, true)
    const allocations = []
    ...
    for (const [name, amount] of Object.entries(plan.withdrawals)) {
      ...
      if (after - before !== amount)
        throw new Error(`Could not retrieve all ${name}; check chest contents and inventory space.`)
      await db('job_status', {
        state: 'running',
    ...
    for (const step of plan.steps) {
      ...
      for (let n = 0; n < step.times; n++) {
        ...
        for (const delta of step.recipe.delta.filter((d) => d.count < 0)) {
          if (w.bot.inventory.items().some((i) => i.type === delta.id && !plain(i)))
            throw new Error('Move modified recipe ingredients out of inventory before crafting.')
          const have = w.bot.inventory
            .items()
            .filter((i) => i.type === delta.id)
            .reduce((sum, i) => sum + i.count, 0)
          if (have < -delta.count) throw new Error('Recipe ingredients changed before crafting.')
        }
        if (w.bot.inventory.emptySlotCount() < 2)
          throw new Error('Keep two empty inventory slots for crafting results.')
        ...
        await storage.inventoryOperation(
          w,
          {
            id: operation,
            kind: 'craft',
            job: job.id,
            item: step.name,
            count: step.recipe.result.count,
          ...
          async () => {
            ...
            w.check()
            await w.timed(
              () => w.bot.craft(step.recipe, 1, table || null),
              20000,
              `Craft ${step.name}`,
            )
    ...
    const output = w.bot.inventory.items().find((i) => i.name === job.item && plain(i))
    const stored =
      output && storeOutput
    ...
    // A read-only status refresh must not replace a critical operation failure.
    if (!w.cancelled()) await storage.list(w).catch(() => {})
    throw error
  }
