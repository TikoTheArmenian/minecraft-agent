...
class ApiCosts {
  constructor({
    file = path.join(__dirname, '..', '..', 'data', 'api-costs.sqlite'),
    now = Date.now,
  } = {}) {
    this.file = file
    this.now = now
  ...
  begin({ agent = 'project', provider, operation, model = null }) {
    ...
    try {
      this.db
        .prepare(
          'INSERT INTO requests (id,started_at,agent,provider,operation,requested_model) VALUES (?,?,?,?,?,?)',
        )
        .run(
          id,
          this.now(),
          word(agent) || 'project',
          word(provider) || 'unknown',
          word(operation) || 'request',
          word(model),
        )
      return id
    } catch (error) {
      this.fault(error, true)
      return null
    }
  }
  finish(id, { outcome = 'completed', httpStatus = null, result = null, requestId = null } = {}) {
    ...
    try {
      ...
      const price =
        row.provider === 'openai'
          ? estimate(model, tier, usage)
          : { costNano: null, pricingStatus: 'provider_billed_separately', price: null }
      const responseId = word(result?.id)
      const duplicate =
        responseId &&
        this.db
          .prepare(
            'SELECT id FROM requests WHERE provider=? AND response_id=? AND duplicate_of IS NULL AND id!=?',
          )
          .get(row.provider, responseId, id)
      this.db
        .prepare(
          `UPDATE requests SET ended_at=?, outcome=?, http_status=?, model=?, tier=?, response_id=?, request_id=?, duplicate_of=?,
        ...
        .run(
          this.now(),
          outcomes.has(outcome) ? outcome : 'failed',
          ...
          duplicate ? 0 : price.costNano,
          duplicate ? 'duplicate' : price.pricingStatus,
          price.price ? JSON.stringify(price.price) : null,
          id,
        )
    } catch (error) {
      this.fault(error, true)
  ...
  totals(filter = {}, group = null) {
    const { where, params } = this.filter(filter),
      stale = this.now() - 60000
    const groups = {
      agent: 'agent',
      model: 'COALESCE(model,requested_model,provider)',
      day: "strftime('%Y-%m-%d',started_at/1000,'unixepoch')",
    }
    const column = groups[group]
    const rows = this.db
      .prepare(
        `SELECT ${column ? `${column} name,` : ''} ${aggregate} FROM requests${where}${column ? ` GROUP BY ${column} ORDER BY name` : ''}`,
      )
    ...
    return rows.map((r) => ({
      ...Object.fromEntries(
        Object.entries(r).map(([key, value]) => [
          key,
          value === null && key !== 'latencyMs' ? 0 : value,
        ]),
      ),
      estimatedUsd: r.costNano / 1e9,
    }))
  }
  records(filter = {}, limit = 50) {
    const { where, params } = this.filter(filter)
    return this.db
      .prepare(`SELECT * FROM requests${where} ORDER BY started_at DESC,id DESC LIMIT ?`)
      .all(...params, limit)
  ...
  publicRecord(row) {
    return {
      ...row,
      outcome:
        row.outcome === 'pending' && row.started_at < this.now() - 60000
          ? 'interrupted'
          : row.outcome,
      estimatedUsd: row.cost_nano === null ? null : row.cost_nano / 1e9,
      price: row.price_json ? JSON.parse(row.price_json) : null,
  ...
  summary(filter = {}, agents = []) {
    ...
    try {
      ...
      for (const agent of agents)
        if (!byAgent.some((r) => r.name === agent)) byAgent.push({ ...empty, name: agent })
      const budgets = this.db.prepare('SELECT * FROM budgets ORDER BY scope').all()
      const alerts = []
      for (const budget of budgets) {
        for (const [period, from, usd] of [
          ['daily', day, budget.daily_usd],
          ['monthly', month, budget.monthly_usd],
        ]) {
          if (usd === null) continue
          const total = this.totals({
            from,
            agent: budget.scope === 'project' ? null : budget.scope,
          })[0]
          if (total.estimatedUsd >= usd * 0.8)
            alerts.push({
              scope: budget.scope,
              period,
              budgetUsd: usd,
              estimatedUsd: total.estimatedUsd,
              level: total.estimatedUsd >= usd ? 'exceeded' : 'approaching',
      ...
      return {
        ...
        trackingSince: Number(
          this.db.prepare("SELECT value FROM metadata WHERE key='started_at'").get().value,
        ),
        ...
        pricing: {
          ...
          source: SOURCE,
          rates,
          stale: now - Date.parse(VERIFIED_AT) > 90 * DAY,
        },
        total: this.totals(filter)[0],
        today: this.totals({ from: day, agent: filter.agent })[0],
        month: this.totals({ from: month, agent: filter.agent })[0],
        agents: byAgent,
        models: this.totals(filter, 'model'),
        daily:
          filter.to && filter.to <= day - 89 * DAY
            ? []
            : this.totals({ ...filter, from: Math.max(filter.from || 0, day - 89 * DAY) }, 'day'),
        recent: this.records(filter),
        budgets,
        alerts,
      }
    } catch (error) {
      this.fault(error)
      return { health: { error: this.error, missedWrites: this.missed }, total: null }
    }
  }
  setBudget({ scope, dailyUsd = null, monthlyUsd = null }) {
    ...
    this.db
      .prepare(
        'INSERT INTO budgets VALUES (?,?,?) ON CONFLICT(scope) DO UPDATE SET daily_usd=excluded.daily_usd,monthly_usd=excluded.monthly_usd',
      )
  ...
  *exportRows(filter) {
    ...
    for (const row of this.db
      .prepare(`SELECT * FROM requests${where} ORDER BY started_at,id`)
      .iterate(...params))
      yield this.publicRecord(row)
  }
  close() {
    this.db?.close()
    this.db = null
...
function costsFor(agent) {
  if (!agent.apiCosts)
    agent.apiCosts = new ApiCosts({ file: path.join(agent.dataDir, 'api-costs.sqlite') })
