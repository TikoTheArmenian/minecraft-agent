...
class LlmChat {
  ...
  async summarize() {
    ...
    const events = (s.logs || [])
      ...
      .filter(Boolean)
      .slice(-30)
    await this.receive(this.agent.bot, 'Marc', 'Summarize the latest 30 seconds.', false, {
  ...
  async receive(bot, username, message, whisper = false, summary = null) {
    if (
      ...
      return
    if (!summary && !whisper && !new RegExp('\\b' + this.agent.username + '\\b', 'i').test(message)) return
    if (!summary && Object.values(this.agent.fleet || {}).some(a => a.username === username)) return
    if (!summary)
      this.agent.log(
        'chat.received',
