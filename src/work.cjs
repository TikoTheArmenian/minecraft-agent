...
class Work {
  ...
  async dig(pos, expected, predicate = () => true, acceptsTool = () => true, safeDescent = () => false) {
    this.check()
    ...
    const confirmation = watchBlock(this.bot, pos, (state) => airStates.includes(state), this.controller.signal)
    try {
      await this.timed(
  ...
  async plant(soilPos, crop) {
    ...
    await this.timed(
      () =>
        this.bot._placeBlockWithOptions(this.bot.blockAt(soilPos), new Vec3(0, 1, 0), {
          forceLook: 'ignore',
          swingArm: 'right',
        }),
      7000,
    ...
    if (this.bot.blockAt(soilPos.offset(0, 1, 0))?.name !== crop.block)
