...
const TARGET = 128
const count = (w, names = BUILDING_BLOCKS) =>
  w.bot.inventory
    .items()
...
async function ensure(w, { names = BUILDING_BLOCKS, threshold = 8 } = {}) {
  ...
  try {
    ...
    if (w.agent.colony?.enabled) await require('../storage/service.cjs').retrieve(w, names, TARGET)
    else {
      await require('./local-supplies.cjs').restockLocal(
        w,
        names,
        TARGET,
        threshold,
    ...
    if (typeof w.gather === 'function') {
      await w.gather(
        ['dirt', 'grass_block'],
        () => count(w, names) >= TARGET,
        `building reserve (${TARGET} blocks)`,
        TARGET,
        false,
      )
    } else {
      ...
      const safe = (b) => {
        if (!b || !['dirt', 'grass_block'].includes(b.name) || b.position.distanceTo(origin) > 80)
          return false
        if (!['air', 'cave_air'].includes(w.bot.blockAt(b.position.offset(0, 1, 0))?.name))
          return false
        for (let x = -4; x <= 4; x++)
          for (let z = -4; z <= 4; z++)
            for (let y = 0; y <= 3; y++)
        ...
        return [
          ...
          const n = w.bot.blockAt(b.position.offset(...d))
          return n && !/water|lava|chest|sign|furnace/.test(n.name)
        })
      }
      const candidates = w.bot.findBlocks({
        matching: ['dirt', 'grass_block'].map((n) => w.bot.registry.blocksByName[n].id),
        maxDistance: 32,
        count: 256,
      })
      let failures = 0
      for (const p of candidates) {
        ...
        try {
          ...
          b = w.bot.blockAt(p)
          if (!safe(b)) continue
          await w.dig(p, b.name, safe)
          await w.pickup(p)
        } catch (error) {
          w.check()
          if (error.fatal) throw error
          failures++
          w.addIssue(error.message)
        }
      }
    }
    if (count(w, names) < TARGET)
      throw new Error(
        `Building supplies: ${count(w, names)}/${TARGET}. Storage and nearby safe gathering could not supply the full batch.`,
      )
  } finally {
    w.refillingBuilding = false
  }
}
