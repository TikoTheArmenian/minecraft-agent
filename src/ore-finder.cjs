...
const { Work, chooseTool } = require('./work.cjs')
...
const ALIASES = { lapis_lazuli: 'lapis', lapis_ore: 'lapis', copper_ore: 'copper', iron_ore: 'iron', coal_ore: 'coal', gold_ore: 'gold', redstone_ore: 'redstone', diamond_ore: 'diamond', emerald_ore: 'emerald' }
...
const oreOf = (name) => ORES.find((ore) => name === `${ore}_ore` || name === `deepslate_${ore}_ore`) || null
// Items that ore blocks drop. Together with coal they form the "batch" that triggers a deposit.
const ORE_ITEMS = new Set(['raw_iron', 'raw_copper', 'raw_gold', 'coal', 'redstone', 'lapis_lazuli', 'diamond', 'emerald'])
// Pickaxe tiers that can be obtained through the existing Survival progression or shared storage.
...
function parseOreFinder(text) {
  ...
  const m = s.match(/^(?:find|mine|search for|locate) ores?(?: (?:for )?([a-z_, ]+?))?(?: within (\d+)(?: blocks)?)?$/)
  if (!m) return null
  ...
  let ores = null
  if (m[1]) {
    ores = [...new Set(m[1].replace(/lapis lazuli/g, 'lapis').split(/[, ]+|\band\b/).map((w) => w.trim()).filter(Boolean).map((w) => ALIASES[w] || w))]
    const unknown = ores.filter((ore) => !ORES.includes(ore))
    if (unknown.length)
      throw new Error(`Unknown ore "${unknown[0]}". Choose from ${ORES.join(', ')} (for example "find ores coal iron").`)
  }
  return { type: 'oreFinder', ores, radius }
}

class OreFinder extends Survival {
  ...
  rename(text) {
    return String(text).replaceAll('Marc', this.name)
  }
  addIssue(text) {
  ...
  safety() {
    ...
    return danger ? this.rename(danger) : null
  }
  check() {
    try {
      super.check()
    } catch (error) {
      error.message = this.rename(error.message)
      throw error
    }
  ...
  async pause(ms = 200) {
    for (let left = ms; left > 0; left -= 250) await Work.prototype.pause.call(this, Math.min(left, 250))
  }
      ...
      for (let x = -4; x <= 4; x++)
        for (let z = -4; z <= 4; z++)
          for (let y = -1; y <= 2; y++) {
            ...
            if (near === 'farmland' || near === 'chest' || near === 'crafting_table' || near === 'furnace') return false
          }
    }
    return true
  }
  ...
  neededPickaxe(blocks) {
    return PICKAXE_TIERS.find((name) => {
      const id = this.bot.registry.itemsByName[name]?.id
      return Number.isInteger(id) && blocks.every((b) => b.canHarvest(id))
    }) || 'iron_pickaxe'
  }
  oreLoad() {
    return this.bot.inventory.items().filter((i) => ORE_ITEMS.has(i.name)).reduce((n, i) => n + i.count, 0)
  }
  carriedPickaxe() {
    return this.bot.inventory.items().filter((i) => /_pickaxe$/.test(i.name)).map((i) => i.name).sort((a, b) => PICKAXE_TIERS.indexOf(b) - PICKAXE_TIERS.indexOf(a))[0] || null
  }
  releaseCooldowns() {
    const now = Date.now()
  ...
  scan() {
    ...
    const matching = this.oreNames().map((n) => this.bot.registry.blocksByName[n]?.id).filter(Number.isInteger)
    const positions = matching.length
      ? this.bot.findBlocks({ matching, maxDistance: this.radius, count: 512 }).filter((p) => p.distanceTo(this.origin) <= 80)
      : []
    const found = {}, nearest = {}, exposedBlocks = []
    let exposed = 0, buried = 0
    const here = this.bot.entity.position
    for (const p of positions) {
      const block = this.bot.blockAt(p), ore = block && oreOf(block.name)
      if (!ore) continue
    ...
    this.agent.log?.(
      'ore.scan',
      `${exposed} exposed / ${buried} buried; nearest exposed: ${Object.entries(nearest).map(([ore, p]) => `${ore} at ${p.x},${p.y},${p.z} (${p.distance} m)`).join(', ') || 'none'}.`,
      'info',
      { taskId: this.id },
    )
    this.agent.publish()
  ...
  summary() {
    const list = Object.entries(this.plan.found).map(([ore, n]) => `${ore} ${n}`).join(', ') || 'none'
  ...
  // Survival crafting progression. Bounded to one attempt per minute.
  async ensureTool(targets) {
    ...
    const names = [...new Set(lacking.map((b) => b.name.replace(/^deepslate_/, '').replace(/_ore$/, '')))].join('/')
    if (Date.now() < this.nextToolAttemptAt) {
      this.addIssue(`Skipping ${lacking.length} ${names} ore(s): a ${needed.replace('_', ' ')} is needed and the last tool trip failed; retrying in a minute.`)
      return
    }
    this.decide(`Need a ${needed.replace('_', ' ')} for ${lacking.length} exposed ${names} ore(s); checking shared storage and crafting.`)
    try {
      if (needed === 'wooden_pickaxe') await this.wooden()
        ...
        if (this.agent.colony?.enabled) await require('./storage.cjs').retrieve(this, ['iron_pickaxe', 'diamond_pickaxe', 'netherite_pickaxe'], 1)
        if (!this.pick('iron')) {
          if (this.count('iron_ingot') < 3)
            throw blocked(`No iron pickaxe in shared storage and only ${this.count('iron_ingot')}/3 iron ingots to craft one.`)
          await this.sticks(2)
          await this.craft('iron_pickaxe', await this.craftingTable())
        }
      }
      this.plan.tool = this.carriedPickaxe()
      if (lacking.some((b) => !this.canMine(b))) throw blocked(`Still no pickaxe that harvests ${names} ore.`)
    } catch (error) {
      if (error.fatal || error.code === 'CANCELLED') throw error
      this.check()
      this.nextToolAttemptAt = Date.now() + TOOL_RETRY_MS
      this.addIssue(`Skipping ${lacking.length} ${names} ore(s) that need a ${needed.replace('_', ' ')}: ${error.message}`)
    }
  }
  shouldStore() {
    return !!this.agent.colony?.enabled && (this.oreLoad() >= BATCH || this.bot.inventory.emptySlotCount() < 4)
  }
  ...
  async deposit(reason) {
    ...
    try {
      const moved = await require('./storage.cjs').store(this)
  ...
  async mineExposed(targets) {
    ...
    for (const block of nearbyFirst(this, targets)) {
      ...
      if (this.bot.inventory.emptySlotCount() < 2)
        throw blocked(this.agent.colony?.enabled
      ...
      this.decide(`Mining ${live.name.replaceAll('_', ' ')} at ${key(live.position)} · ${this.counts.mined} ores mined, ${this.plan.stored} stored.`)
      ...
      if (await this.harvest(live)) {
        ...
        if (/route/i.test(this.issues.slice(issuesBefore - this.issues.length || undefined).join(' ')))
  ...
  async run(command = {}) {
    this.ores = Array.isArray(command.ores) && command.ores.length ? command.ores.filter((o) => ORES.includes(o)) : null
    ...
    try {
      while (true) {
        ...
        try {
          ...
          if (this.stalledPasses >= 3)
            throw new Error(`No progress after three passes. ${error.message} Fix the blocker, then restart the ore finder.`)
      ...
      this.plan.decision = `Ore finder ${this.plan.status}: ${this.rename(reason.message)}`
