...
function normalizePassagePath(bot, result) {
  for (const p of result.path || []) {
    ...
    p.x = lower.x + .5; p.y = lower.y; p.z = lower.z + .5
  }
}
function installPassages(bot) {
  ...
  bot.activateBlock = async (block, ...args) => {
    ...
    const pos = block.position.clone(), name = block.name
    const current = () => {
      if (bot.pathfinder.goal !== goal) throw new Error('Passage opening cancelled.')
      const b = bot.blockAt(pos)
      if (b?.name !== name || !handOpenable(b)) throw new Error('Passage changed or requires redstone.')
      if (bot.entity.position.offset(0, 1.62, 0).distanceTo(pos.offset(.5,.5,.5)) > 4.5) throw new Error('Passage is out of reach.')
      return b
    }
    if (current().getProperties().open) return
    await bot.lookAt(pos.offset(.5,.5,.5))
    if (current().getProperties().open) return
    const cells = name.endsWith('_door') ? [pos, pos.offset(0,1,0)] : [pos]
    const acknowledgements = cells.filter(p => !bot.blockAt(p)?.getProperties().open).map(p => watchBlock(bot, p, state => {
    ...
    try {
      const cancelled = new Promise((_, reject) => {
        changed = next => { if (next !== goal) reject(new Error('Passage opening cancelled.')) }
      ...
      bot._client.write('block_place', { location: pos, direction: 1, hand: 0, cursorX: .5, cursorY: .5, cursorZ: .5, insideBlock: false, sequence: 0 })
      bot.swingArm?.('right')
      await Promise.race([Promise.all(acknowledgements.map(a => a.promise)), cancelled])
      current()
      if (cells.some(p => bot.blockAt(p)?.name !== name || !bot.blockAt(p).getProperties().open)) throw new Error('Passage closed before crossing.')
    } catch (error) {
