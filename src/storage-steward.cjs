...
async function tools(w, hub) {
  for (const name of TOOL_STOCK) {
    ...
    const count=fresh.containers.filter(c=>c.managed && atHub(c,hub)).flatMap(c=>c.slots).filter(i=>i.name===name && plain(i)).reduce((n,i)=>n+i.count,0)
    if(count>=4) continue
...
module.exports={atHub,destinations,consolidate,label,tools,signText,TOOL_STOCK}
