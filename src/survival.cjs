/**
 * STARTER SKILL: builds supplies step by step (wood, tools, iron, food, wheat).
 * Also provides reusable inventory, recipe, gathering, and safety helpers to other skills.
 * Read run() for the overall sequence, then the individual milestone methods.
 */
...
class Survival extends Work {
  ...
  check() {
    if (!this.cancelled() && this.bot.game.gameMode !== 'survival')
      throw Object.assign(
        blocked('Marc left Survival mode. Switch it back before restarting the routine.'),
        { fatal: true },
      )
  ...
  safety() {
    if (this.bot.health <= 6)
      return 'Health is critically low. Move Marc to safety and provide food before continuing.'
    ...
    if (Number.isFinite(this.bot.oxygenLevel) && this.bot.oxygenLevel < 8)
      return 'Air is running low. Bring Marc out of the water.'
  ...
  async gather(
    ...
    while (!satisfied() && removed < maxBlocks && rounds++ < 12) {
      ...
      let changed = false
      for (const block of require('./work-order.cjs').nearbyFirst(this, targets)) {
  ...
  async craft(name, table = null) {
    ...
    if (this.agent.colony?.enabled && /_(pickaxe|axe|shovel|hoe|sword)$/.test(name)) {
      ...
      await require('./storage.cjs').retrieve(this, [name], target)
  ...
  async placeItem(name, support) {
    ...
    await this.timed(
      () =>
        this.bot._placeBlockWithOptions(current, new Vec3(0, 1, 0), {
          forceLook: 'ignore',
          swingArm: 'right',
        }),
      7000,
  ...
  async food() {
    ...
    if (this.foodCount() < 1)
      throw blocked(
        'No reachable food found. The new farm can supply food after it grows; apples, bread, or other food can also be dropped beside Marc.',
      )
  ...
  async till(pos, hoe) {
    ...
    try {
      ...
      await this.timed(
        () =>
          this.bot._genericPlace(soil, new Vec3(0, 1, 0), {
            forceLook: 'ignore',
            swingArm: 'right',
          }),
        5000,
  ...
  async starterFarm() {
    ...
    if (cluster)
      ...
      if (!spots.length)
        throw blocked(
          'Need four clear dirt/grass plots near existing water. Choose a shoreline on the map, move Marc nearby, and start again.',
        )
  ...
  async collectWheat(needed) {
    ...
    while (this.count('wheat') < needed) {
      ...
      for (const name of ['attempt', 'extendShore', 'irrigationRemains'])
        expansion[name] = methods[name]
      try {
        await methods.expand.call(expansion)
      } catch (error) {
        if (error.fatal || error.code === 'CANCELLED') throw error
