...
async function main() {
  ...
  if (argument !== undefined) {
    ...
    let log = ''
    try { log = fs.readFileSync(logPath, 'utf8') } catch (err) {
    ...
    if (!matches.length) throw new Error('Open Agent Playground to LAN first, then run npm start again. Or specify its port: npm start -- 53124')
  ...
  const status = await ping({ host: '127.0.0.1', port, version: '1.21.1', closeTimeout: 3000, noPongTimeout: 1000 })
  // Protocol 767 is shared by Minecraft 1.21 and 1.21.1; the profile selects 1.21.1.
  if (status.version?.protocol !== 767) throw new Error(`Server reports ${status.version?.name}. Launch Agent Test with Minecraft 1.21.1.`)
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (existing.auth !== 'offline' || existing.host !== '127.0.0.1') {
      throw new Error('config.json contains a nonlocal setup. Run node bot.cjs to use it, or move that config aside before starting the local demo.')
    }
  }
  ...
  fs.writeFileSync(configPath, JSON.stringify({ host: '127.0.0.1', port, username: 'Marc', auth: 'offline', version: '1.21.1' }, null, 2) + '\n', { mode: 0o600 })
