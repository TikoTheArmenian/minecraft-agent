/**
 * ITEM PICTURES: maps inventory items to extracted Minecraft textures when available.
 * Text labels remain usable even if textures cannot be loaded.
 */
...
;(() => {
  ...
  function face(cls, spec, tint) {
    ...
    if (typeof spec === 'string') el.style.setProperty('--img', `url("/textures/${spec}")`)
      ...
      canvas.width = spec.size[0]
      canvas.height = spec.size[1]
      el.append(canvas)
      loadImage(spec.file)
        .then((img) => {
          const ctx = canvas.getContext('2d')
          ctx.imageSmoothingEnabled = false
          for (const [sx, sy, sw, sh, dx, dy] of spec.draw)
  ...
  function paint(el) {
    ...
    if (!entry) {
      el.classList.add(settled ? 'missing' : 'pending')
      el.textContent = settled ? initials(name) : ''
      return
    }
    if (entry.type === 'cube') {
      ...
      cube.append(
        face('top', entry.top, entry.tintTop),
        face('front', entry.front ?? entry.side, entry.tintSide),
        face('right', entry.side, entry.tintSide),
      )
      el.append(cube)
  ...
  function chip(item) {
    ...
    el.append(
      icon(item.name),
      create('span', undefined, `${item.displayName || label(item.name)} × ${item.count}`),
    )
    if (item.enchantments?.length)
      el.title = item.enchantments
        .map((e) => `${label(e.name)} ${e.lvl ?? e.level ?? ''}`.trim())
        .join(', ')
    return el
  }
  fetch('/textures/manifest.json', { cache: 'force-cache' })
    ...
    .finally(() => {
      settled = true
      document.querySelectorAll('.item-icon').forEach(paint)
    })
